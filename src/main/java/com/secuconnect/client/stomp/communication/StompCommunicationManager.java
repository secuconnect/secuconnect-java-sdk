...
import com.secuconnect.client.stomp.listeners.IFrameListener;
import com.secuconnect.client.stomp.model.AuthSessionsRefresh;
import com.secuconnect.client.stomp.model.StompRequest;
...
public class StompCommunicationManager {
    ...
    public void connect(int timeoutInMillis) throws RuntimeException {
        ...
        sender.sendFrame(StompFrame.createFromData(FrameCommands.Request.CONNECT, getConnectionHeaders(), ""));
    ...
    public void send(String destination, String body, String receipt) throws RuntimeException {
        ...
        headers.put(FrameHeaders.Request.Send.DESTINATION, properties.getProperty("base-destination") + destination);
        if (receipt != null) headers.put(FrameHeaders.Request.RECEIPT, receipt);

        if (body == null) {
            body = "";
        }

        sender.sendFrame(StompFrame.createFromData(FrameCommands.Request.SEND, headers, body));
    }
    ...
    private Map<String, String> getConnectionHeaders() {
        ...
        headers.put(FrameHeaders.Request.Connect.REPLY_TO, properties.getProperty("reply-to"));
        headers.put(FrameHeaders.CONTENT_TYPE, properties.getProperty("content-type"));
        headers.put(FrameHeaders.Request.Connect.CORRELATION_ID, properties.getProperty("correlation-id"));
        headers.put(FrameHeaders.Request.Connect.CLIENT_ID, accessToken);
        headers.put(FrameHeaders.Request.Connect.USER_ID, accessToken);
        headers.put(FrameHeaders.Request.Connect.LOGIN, accessToken);
        headers.put(FrameHeaders.Request.Connect.PASSCODE, accessToken);
        return headers;
    }
